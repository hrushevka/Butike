...
using last_test_server.Services;
...
using (var scope = app.Services.CreateScope())
{
    ...
    catch(Exception ex)
    {
        ...
        logger.LogError(ex, "Ошибка в подключении в БД");
    }
}
...
public class RussianIdentityErrorDescriber : IdentityErrorDescriber
{
    ...
    { Code = nameof(InvalidEmail), Description = $"Email {email} имеет не верный формат" };
    ...
    { Code = nameof(InvalidRoleName), Description = $"Название роли: {role} недопустимо" };
    ...
    { Code = nameof(PasswordRequiresNonAlphanumeric), Description = $"Пароль должен содержать хотя бы один символ " };
    public override IdentityError PasswordTooShort(int lenght) => new IdentityError
    { Code = nameof(PasswordTooShort), Description = $"Пароль должен содержать не менне:{lenght} символов " };
    ...
    { Code = nameof(UserNotInRole), Description = $"Пользователь не имеет роль:'{role}'" };
